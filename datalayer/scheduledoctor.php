...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (!isScheduleUnique($mysqli, $doctorId, $date, $startTime, $endTime)) {
        echo "<script>window.alert('This schedule already exists for the selected doctor.'); window.location.href='../applicationlayer/doctor/schedule.php';</script>";
        exit();
    }
    ...
    if($result==TRUE){
        echo "<script>window.alert('Successfully registered'); window.location.href='../applicationlayer/doctor/schedule.php';</script>";
    } else {
        echo "<script>window.alert('Registration fail!!'); window.location.href='../applicationlayer/doctor/schedule.php';</script>";
    }  
}
?>